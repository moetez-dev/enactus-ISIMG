/* ================================================================
   ...
   Handles: AOS init, navbar scroll, mobile menu, back-to-top,
            progress bar, team grids, project/member modals,
            Firestore events timeline, EOM display.

   Script load order (index.html):
     1. tailwind play cdn
     2. aos.css + aos.js
     3. lucide
     4. firebase sdks (app / auth / firestore)
...
var ENACTORS = [
  ...
  { name: "Khadija Samir",     role: "Active Member", img: "enactuss/khadija-samir.png",                                    fb: "#", ig: "#" }
];
...
function fillGrid(data, gridId) {
  ...
  data.forEach(function (member) {
    ...
    function activate() { showMember(member); }
    ...
    imgWrapper.className = "rounded-[2rem] overflow-hidden border-2 border-transparent group-hover:border-yellow group-hover:shadow-lg transition-all duration-300 mb-3 aspect-square";
    ...
    img.onerror   = function () {
      img.src = "https://ui-avatars.com/api/?name=" + encodeURIComponent(member.name) +
                "&background=111&color=FFC222&size=200";
    };
    imgWrapper.appendChild(img);

    var nameEl = document.createElement("p");
    nameEl.className   = "text-[9px] font-black uppercase tracking-widest truncate px-1 text-white";
...
function showMember(m) {
  ...
  if (mImg) {
    ...
    mImg.onerror = function () {
      mImg.src = "https://ui-avatars.com/api/?name=" + encodeURIComponent(m.name) +
...
function loadEOM() {
  db.collection("settings").doc("eom").onSnapshot(
    ...
    function (err) { console.warn("[loadEOM]", err.message); }
  );
}
...
function loadTimeline() {
  ...
  /* Order by createdAt descending — no composite index needed */
  db.collection("events")
    ...
    .onSnapshot(
      function (snapshot) {
        ...
        snapshot.forEach(function (doc, index) {
          ...
          row.className = "relative flex items-center " +
            (isEven ? "md:flex-row-reverse justify-end" : "justify-start") + " group";
      ...
      function (err) {
        container.innerHTML =
          '<div class="text-center text-gray-400 text-xs py-10">Could not load events. Please try again later.</div>';
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  var projFytrlance = document.getElementById("proj-fytrlance");
  if (projFytrlance) {
    projFytrlance.addEventListener("click", function () { openProject("fytrlance"); });
    projFytrlance.addEventListener("keydown", function (e) {
      if (e.key === "Enter" || e.key === " ") { e.preventDefault(); openProject("fytrlance"); }
    });
  }

  var projBioverto = document.getElementById("proj-bioverto");
  if (projBioverto) {
    projBioverto.addEventListener("click", function () { openProject("bioverto"); });
    projBioverto.addEventListener("keydown", function (e) {
      if (e.key === "Enter" || e.key === " ") { e.preventDefault(); openProject("bioverto"); }
    });
  }

  /* Modal: close on overlay click or Escape key */
  var modal = document.getElementById("unifiedModal");
  if (modal) {
